...
const menu = {
  ...
  contents: (wrapper, Html, core, menu, args) => {
    ...
    Args = args;
    console.log(args);
    menu.title(`Group chat with ${args.characters.length} characters`);
    wrapper.styleJs({
      display: "flex",
        ...
        if (message.role != "function") {
          if (message.content) {
            createBubble(
              ...
              false,
              false,
              message.character ? message.character : null
            );
          }
    ...
    typingCb = (e) => {
      let data = e.detail;
      createBubble(`${data.characterName} is typing...`, false, () => {
        console.log("no cb yet");
      });
    };
